...
int argmax(double * tab,int count){
    int m=0;
    for(int i=1;i<=count-1;i++){
        if(tab[i]>tab[m]){
            m=i;
...
int main(){
    ...
    int largeur_echantillons_audio = 30000 ; 
    ...
    for(int k=0;k<=compteur-1;k++){
        ...
        freq_max[k]=argmax(temporaire,largeur_echantillons_audio);
        free(temporaire);
    }
    ...
    for(int numero_echantillon_audio=1;numero_echantillon_audio<nb_echantillons_audio;numero_echantillon_audio++){
        ...
        int freq_max_test = argmax(transformee,largeur_echantillons_audio);
        ...
        for(int i=0;i<=compteur-1;i++){
            if(absolue(freq_max[i],freq_max_test)<10){
                printf("%s ",fichiers[i]);
            }
        }
        printf("\n");
...
// gcc morceau_methode3.c -lm -lgsl -lgslcblas -lsndfile